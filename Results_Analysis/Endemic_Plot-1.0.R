...
p + geom_violin() +
  geom_jitter(aes(colour=Area),width=0.7, height = 0, size=2)+
...
p + geom_violin() +
  geom_jitter(aes(colour=Area), width = .7,height = 0, size=2)+
  #geom_point(aes(x=factor(Percentage),y=mean(Values)),colour="red",shape=17,size=4)+
...
col2 <- col[-c(grep("Paramo", names(col)),grep("Cauca", names(col)),grep("Magdalena", names(col)),grep("WEcuador", names(col)))]

endAVTD <- end[-c(grep("Cauca", end$Area),grep("Paramo", end$Area),grep("Magdalena", end$Area),grep("WEcuador", end$Area)), ]
endAVTD
